...
{
    public class FibPaymentService(HttpClient httpClient, string baseUrl, string clientId, string clientSecret)
        : IFibPaymentService
    {
        ...
        public async Task<FibAuthorizationResponse> GetAccessTokenAsync()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, $"{baseUrl}/auth/realms/fib-online-shop/protocol/openid-connect/token")
            {
        ...
        {
            var response = await MakeAuthorizedRequestAsync(HttpMethod.Post, $"{baseUrl}/protected/v1/payments", request);
        ...
        {
            var response = await MakeAuthorizedRequestAsync(HttpMethod.Get, $"{baseUrl}/protected/v1/payments/{paymentId}/status");
        ...
        {
            await MakeAuthorizedRequestAsync(HttpMethod.Post, $"{baseUrl}/protected/v1/payments/{paymentId}/cancel");
        }

        public async Task RefundPaymentAsync(string? paymentId)
        {
            await MakeAuthorizedRequestAsync(HttpMethod.Post, $"{baseUrl}/protected/v1/payments/{paymentId}/refund");
        }
        ...
        public async Task PollPaymentStatusAsync(string paymentId, int intervalInSeconds, int timeoutInSeconds, Func<FibPaymentStatusResponse, Task> onStatusUpdate)
        {
            ...
            while (DateTime.UtcNow < timeout)
            {
                ...
                if (status?.Status is "PAID" or "DECLINED")
                {
